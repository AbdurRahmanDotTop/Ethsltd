    ...
    <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
      ...
      <div className="mb-6">
        ...
        <div className={`flex items-center gap-2 mt-2 text-sm font-medium ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <span>
            {isPositive ? '+' : ''}${Math.abs(summary.change24hUsd).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
      ...
      <div className="grid grid-cols-2 gap-4 pt-6 border-t border-border">
        <div>
          ...
          <div className="text-lg font-semibold text-foreground">
            ${summary.availableBalanceUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
        <div>
          <div className="text-sm text-muted-foreground mb-1">Locked Funds</div>
          <div className="text-lg font-semibold text-foreground">
            ${summary.lockedBalanceUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
      </div>
    </div>
