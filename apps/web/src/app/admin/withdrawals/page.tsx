...
export default function AdminWithdrawalsPage() {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      if (res.success) fetchWithdrawalsCurrent();
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex flex-col">
          <span className="text-xs font-medium text-muted-foreground">{(row.network as string) || 'Internal'}</span>
          <span className="text-xs font-mono bg-muted px-1.5 py-0.5 rounded truncate max-w-[150px]" title={row.address as string}>
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground truncate max-w-[150px]" title={row.reference as string}>
            {(row.reference as string) || "-"}
          </span>
          <button onClick={() => handleEditNotes(row.id as string, row.reference as string)} className="text-muted-foreground hover:text-foreground">
    ...
    },
    {
      ...
      accessor: (row) => {
        ...
        const color = colors[row.status as string] || "bg-muted text-foreground border-border";
        return (
    ...
    {
      header: "Date",
      accessor: (row) => <span className="text-xs text-muted-foreground">{new Date(row.createdAt as string).toLocaleString()}</span>
    },
    {
      header: "Actions",
      accessor: (row) => {
        return (
          <div className="flex gap-2">
            {row.status === "PENDING" && (
              <>
                <button onClick={() => handleApprove(row.id as string)} className="p-1 rounded bg-green-500/10 text-green-500 hover:bg-green-500/20 transition-colors" title="Approve">
                  <Check className="w-4 h-4" />
                </button>
                <button onClick={() => handleReject(row.id as string)} className="p-1 rounded bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors" title="Reject">
                  <X className="w-4 h-4" />
                </button>
              </>
            )}
            {user?.role === 'SUPER_ADMIN' && (
              <button onClick={() => handleDelete(row.id as string)} className="p-1 rounded bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors" title="Delete completely">
