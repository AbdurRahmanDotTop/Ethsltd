...
init_checked_fetch();
init_modules_watch_stub();
var kycProfiles = sqliteTable("kyc_profiles", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
init_checked_fetch();
init_modules_watch_stub();
var ledgerAccounts = sqliteTable("ledger_accounts", {
  id: text("id").primaryKey(),
...
var ledgerTransactions = sqliteTable("ledger_transactions", {
  id: text("id").primaryKey(),
...
init_checked_fetch();
init_modules_watch_stub();
var markets = sqliteTable("markets", {
  id: text("id").primaryKey(),
...
init_checked_fetch();
init_modules_watch_stub();
var p2pAds = sqliteTable("p2p_ads", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id),
...
var p2pOrders = sqliteTable("p2p_orders", {
  id: text("id").primaryKey(),
...
var p2pMessages = sqliteTable("p2p_messages", {
  id: text("id").primaryKey(),
...
var ticketMessages = sqliteTable("ticket_messages", {
  id: text("id").primaryKey(),
...
walletRoutes.get("/transactions", async (c) => {
  const db = c.get("db");
  const user = c.get("user");
  const transactions = await db.select().from(walletTransactions).where(eq(walletTransactions.userId, user.id)).orderBy(desc(walletTransactions.createdAt)).all();
...
tradingRoutes.get("/orders", async (c) => {
  const db = c.get("db");
  const user = c.get("user");
  const userOrders = await db.select().from(orders).where(eq(orders.userId, user.id)).orderBy(desc(orders.createdAt)).all();
...
adminRoutes.get("/stats", async (c) => {
  ...
  try {
    ...
    const totalP2pAds = await db.select({ count: sql`count(*)` }).from(p2pAds).where(eq(p2pAds.status, "ACTIVE"));
    const pendingKyc = await db.select({ count: sql`count(*)` }).from(users).where(eq(users.status, "PENDING_VERIFICATION"));
    return c.json({
      success: true,
      data: {
  ...
  try {
    ...
    if (search) {
      ...
      allUsers = allUsers.filter(
        (u) => u.email.toLowerCase().includes(s) || u.displayName && u.displayName.toLowerCase().includes(s) || u.id.toLowerCase().includes(s)
      );
    }
