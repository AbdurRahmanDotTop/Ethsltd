...
## 1. Overview
The platform's current "Paper Trading" feature will be rebranded and expanded into a comprehensive "Demo Trading" feature. The primary objective is to separate "Real Trading" and "Demo Trading" entirely across all modules, including Normal (Spot) Trading, P2P Trading, Wallet Balances, Orders, and Transaction History. This ensures a fully isolated, risk-free environment for users to practice every feature on the platform.

## 2. Terminology Changes
- "Paper Trading" -> "Demo Trading"
- "Paper Wallet" -> "Demo Wallet"
- Internal variables (e.g., `PAPER`) -> `DEMO`
- URLs (e.g., `/learn/paper-trading`) -> `/learn/demo-trading`

## 3. Data Isolation Requirements
To achieve complete separation between Real and Demo environments:
1. **API Client (`X-Trading-Mode`)**: The `mode` header will now send `DEMO` instead of `PAPER`. The backend logic will handle isolated databases/namespaces for Demo transactions.
2. **P2P Marketplace**:
...
- **Badges**: All "PAPER TRADING" badges will update to "DEMO TRADING".
- **Learn Section**: All educational content referencing Paper Trading will be updated to Demo Trading.
